...
class scene {
 public:
  ...
  const sceneGraph_base& addSceneGraphObject(const std::vector<triangle>& triangles, const material& mat, bool computeBVH=false);
  const sceneGraph_base& addSceneGraphObject(const std::string& filename, const material& mat, bool computeBVH=false);
 ...
 private:
  ...
  std::vector<boundingVolumeHierarchy*> _bvh;
};
