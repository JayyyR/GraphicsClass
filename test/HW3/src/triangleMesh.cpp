...
intersectionPoint triangleMesh::intersect(const ray& r) const
{
  ...
  //      Input: a ray
  //      Modifies: nothing
  //      Returns: The intersection point with the ray r and the triangle mesh which is closest to the origin of the ray.
  //               If no intersection point is found, return an empty intersectionPoint (i.e., construct without parameters) 
  
  //check every triangle
  float smallestT = std::numeric_limits<float>::max();
  vec3d finalBary;
  triangle finalTriangle;
  //cout<<"smallest T is: " << smallestT <<endl;
  bool wasHit = false;
  for(std::vector<triangle>::const_iterator itr = _triangles.begin(); itr != _triangles.end(); itr++)
  {
    float t;
    ...
    if(itr->intersect(r, barycentric, t)){
      ...
      if (t<smallestT){
	smallestT = t;
	finalBary = barycentric;
	finalTriangle = *itr;
	//cout<<"setting smaller t"<<endl;
	
      }
    }
  }
  if (wasHit){
   //cout<<"smalest T is: " << smallestT <<endl;
    intersectionPoint point = intersectionPoint(r, finalBary, smallestT, finalTriangle, _material);
    return point;
  }
